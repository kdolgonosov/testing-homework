...
## Запуск и комментарий

Для запуска тестов (сli):
```
npm test
```
...
npm run test:bug
```
Для изменения BUG_ID необходимо менять значение переменной в test/testplane/utils.ts. Для запуска тестов без BUG_ID - присвойте значение null переменной. Если будут какие-либо вопросы - пишите в тг @kdolgonosov
